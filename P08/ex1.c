...
{
	/* complexidade do algoritmo: no pior dos casos a compleidade vai ser O(n) */
	if(no==NULL ) return 0;

    if(no->str[0]==inicio) return 1 + procura_inicio(no->direita, inicio) + procura_inicio(no->esquerda, inicio);
    else if(no->str[0]>inicio) return procura_inicio(no->esquerda, inicio);
    else return procura_inicio(no->direita, inicio);
